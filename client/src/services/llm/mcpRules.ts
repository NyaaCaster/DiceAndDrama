/**
 * MCP 工具数据使用准则。Dice & Drama 仅启用 NyaaChat-MCP 的 `roll_dnd`
 * —— D&D 风奇幻冒险只需要"d20 + 修正 vs DC"这一种判定语义，CoC 调查
 * 向的 `roll_coc` 与无判定语义的通用 `roll_dice` 都不在本游戏使用范围内。
 *
 * 三层结构：
 *   1. 全局头：列出实际广告出来的工具 + 失败降级规则
 *   2. 掷骰通则：明骰 / 暗骰判断、结果四步重排格式
 *   3. 单工具规则：roll_dnd 链式调用约束
 *
 * 规则来源：NyaaChat-MCP README §1（骰子无状态告警）+ §2.8（roll_dnd 工具说明）。
 *
...
3. 再突出展示**判定结果**（如"**vs DC 15 → 成功**"、"**暴击**"）。
4. 最后用次级文字简述判定依据（对比 DC / 优劣势取舍）。

格式参考（roll_dnd 力量检定 1d20+3 vs DC 15、最终 17）：
> **🎲 17**
> 1d20 投出 14，加力量修正 +3。
> **✨ vs DC 15 → 成功**
...
- **暗骰**：DM 主动触发的、玩家提前知道结果会破坏沉浸感的检定（隐藏的感知 / 识谎 / 潜行被发现 / 命运豁免等）→ 不公开具体骰点，用【暗骰】标记或仪式语引出（如"（暗中掷骰）"），只告知判定结果或直接把结果叙事化呈现。
...
- DnD 攻击 vs DC **失败** → **不要**再追加任何掷骰；失败时直接叙述未命中。
...
const DICE_TOOLS = new Set(["roll_dnd"]);
